...
# --- CONFIGURATION ---
from ..config import VLLM_BINARY, MODEL_PATH, LOG_FILE, VLLM_LOG_FILE, VLLM_PORT, API_PORT, VLLM_HOST, VLLM_SERVER_ARGS
...
async def lifespan(app: FastAPI):
    ...
    vllm_process = await asyncio.create_subprocess_exec(
        VLLM_BINARY, "serve", *VLLM_SERVER_ARGS,
        stdout=vllm_log_file, stderr=vllm_log_file
    )
    ...
    try:
        ...
        if vllm_process and vllm_process.returncode is None:
            ...
            except ProcessLookupError: pass # Le processus peut déjà être terminé
        vllm_log_file.close()
...
async def chat_relay(history: list, patient_id: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(f"http://{VLLM_HOST}:{VLLM_PORT}/v1/chat/completions", json=payload, timeout=120.0)
            response.raise_for_status()  # Lève une exception pour les codes d'erreur HTTP (4xx, 5xx)
        except (httpx.RequestError, httpx.HTTPStatusError) as e:
            print(f"❌ Erreur de communication avec le moteur vLLM: {e}")
            raise HTTPException(status_code=503, detail="Le service d'inférence est indisponible.")
        ...
        no_think_response = re.sub(r'<think>.*?</think>', '', decoded_response, flags=re.DOTALL)
        ...
        log_entry = {"audit_id": str(uuid.uuid4()), "patient_id": patient_id, "out": ai_response, "ts": time.time()}
