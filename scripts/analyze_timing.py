...
def analyze(file_path: str):
    ...
    print(f"{'-'*50}")
    print(f"{'TOTAL (Réseau)':<20} {statistics.mean(total_latencies):>10.0f} {statistics.median(total_latencies):>10.0f} {percentile(total_latencies, 95):>10.0f}")
    
    for c, vals in stats.items():
        if not vals: continue
        print(f"{c:<20} {statistics.mean(vals):>10.0f} {statistics.median(vals):>10.0f} {percentile(vals, 95):>10.0f}")

if __name__ == "__main__":
    file_path = sys.argv[1] if len(sys.argv) > 1 else "scripts/benchmark/data/results_cloud.jsonl"
    analyze(file_path)
