...
def enrich_example(example):
    ...
    enriched["chosen"] = (
        f"{example['chosen']} (Note clinique détaillée : le triage a été effectué selon les protocoles du CHSA, en tenant compte des antécédents du <PATIENT>)."
    )
    enriched["rejected"] = (
        f"{example['rejected']} (Réponse trop brève, ne respectant pas les protocoles de traçabilité.)"
    )
    return enriched
...
def generate_synthetic_dataset(input_path, output_path, multiplier=3):
    ...
    print(
        f"✅ Dataset synthétique créé : {len(synthetic_data)} paires dans {output_path}"
    )
...
if __name__ == "__main__":
    ...
    output_file = (
        "data/processed/Mpaga_Christophe_1_Dataset_DPO_Final_Medical_Enriched.jsonl"
    )
