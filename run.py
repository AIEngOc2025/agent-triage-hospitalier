...
def main():
    ...
    if IS_PRODUCTION:
        ...
        port = os.getenv(
            "PORT", "8001"
        )  # Le port 8001 est standard pour les conteneurs
        workers = str(
            os.getenv("WEB_CONCURRENCY", "4")
        )  # Gunicorn attend une chaîne de caractères pour le nombre de workers
        ...
        command = [
            ...
            "--preload",
            "-b",
            f"0.0.0.0:{port}",  # Bind to all network interfaces
            APP_ENTRYPOINT,  # Application entry point (e.g., "src.api.orchestrateur:app")
        ]
        print(
            f"🚀 Lancement du serveur de PRODUCTION avec la commande : {' '.join(command)}"
        )
        subprocess.run(command)
    else:
        # Commande pour le développement local avec Uvicorn
        print(
            f"🚀 Lancement du serveur de DÉVELOPPEMENT sur http://127.0.0.1:{API_PORT}"
        )
